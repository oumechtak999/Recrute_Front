...
@Pipe({ name: 'CandidatsFilterByPrenom' })
export class CandidatsFilterByPrenomPipe implements PipeTransform {
  ...
  transform(items: OffreCandidat[], searchText: string): any[] {
    ...
    return items.filter(it => {
      return it.candidat.prenom.toLocaleLowerCase().includes(searchText);
    });
  }
}
