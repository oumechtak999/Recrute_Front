...
@Pipe({ name: 'CandidatsFilterByTelephone' })
export class CandidatsFilterByTelephonePipe implements PipeTransform {
  ...
  transform(items: OffreCandidat[], searchText: string): any[] {
    ...
    return items.filter(it => {
      return it.candidat.telephone.toLocaleLowerCase().includes(searchText);
    });
  }
}
