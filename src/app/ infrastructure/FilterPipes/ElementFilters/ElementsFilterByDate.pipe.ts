...
@Pipe({ name: 'ElementsFilterByDate' })
export class ElementsFilterByDatePipe implements PipeTransform {
  ...
  transform(items: Element[], searchText: string): any[] {
    ...
    return items.filter(it => {
      return it.date.toString().toLocaleLowerCase().includes(searchText);
    });
  }
}
